    ...
    <Link href="/conta" className={`trial-banner trial-banner-${tone}`}>
      ...
      <span>
        {expired ? (
          ...
          <>Amanhã termina o seu trial <strong>Pack Plus + IA</strong>. Subscreva em <strong>A minha conta</strong> para manter os dados →</>
        ) : (
          <>A experimentar <strong>Pack Plus + IA</strong> · faltam <strong>{daysLeft} dia{daysLeft !== 1 ? 's' : ''}</strong> · gerir em <strong>A minha conta</strong> →</>
        )}
      </span>
      {tone === 'info' && (
